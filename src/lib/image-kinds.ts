// The image-kind registry: every "page image" we render from SVG via resvg lives
// here as data. Adding a new kind (square-for-Discover, blank variant, AI-fill) is
// one new entry — no new plumbing. `pinSvg`/`wrapTitle` are migrated verbatim from
// the old scripts/pinterest/pin-template.mjs (byte-parity guarded by a test).
import { chartEligible, chartModelForPage, chartMarkForPage, chartSvg, type ChartPageData } from './chart-image'

// === migrated verbatim from scripts/pinterest/pin-template.mjs ===
const INDIGO = '#4338ca'
...
export function wrapTitle(title: string, maxChars = 17, maxLines = 4): string[] {
...
export function pinSvg({ title }: { title?: string }): string {
...
export type ImageKind = {
  ...
  width: number
  height: number
  eligible: (d: ChartPageData) => boolean
  svg: (d: ChartPageData) => string
}

export const IMAGE_KINDS: Record<'pin' | 'chart', ImageKind> = {
  pin: {
    dir: 'pins',
    width: W,
    height: H,
    eligible: () => true, // every live page is pinnable (matches make-pins today)
    svg: (d) => pinSvg({ title: d.h1 }),
  },
  chart: {
    dir: 'charts',
    width: 1650,
